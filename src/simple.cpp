...
std::vector<LL> sparseDP(const PathCover &pc, const std::vector<Anchor> &A) {
	// computeMPCIndex(pc);
	LL K = pc.size();
	...
	for (LL vidx = 0; vidx < N; vidx++) {
		...
		// std::cout<<"step 1"<<endl;
			...
			for (LL j : ids) {
				...
				// std::cout << "type 1 " << j << ": " << C[j].first << " " << C[j].second << std::endl;
				}
				if (A[j].path.back() == v) {
	...
	// std::cout << "optimal ends : " << best.second << std::endl;
	std::vector<LL> ret;
	for (LL i = best.second; i != -1; i = C[i].second) {
		ret.push_back(i);
		// std::cout << "now " << i << "  " << C[i].first << " "  << C[i].second << endl;
		if (i == C[i].second) {
			std::cout << "error, loops in C[j] : " << i << "  " << C[i].first << " "  << C[i].second << endl;
			break;
		}
	}
...
bool checkMinPathCover(const PathCover &pc) {
	std::vector<LL> ids;
...
int main(int argc, char *argv[]) {
	...
	std::string graph_filename = "", anchor_filename = "", output_filename = "";
	if (argc > 1) {
		auto peek = [&](int idx) {
			if (idx < argc)
		...
		std::cout << "usage : ./a -f graph.input.file -a anchor.input.file -o output.file";
	}
	loadGraphFile(graph_filename);
	...
	if (anchor_filename != "") anchor_sets = loadAnchorSets(anchor_filename);
	LL anchor_count = 0;
	for (auto a : anchor_sets) anchor_count += a.anchors.size();
	std::cout << "Loaded " << anchor_sets.size() << " anchor sets, " << anchor_count << " in total" << std::endl;
	{ // map ids
	...
	if (!checkMinPathCover(opt)) 
		std::cout << "error : not a minimum path cover!!" << std::endl;
	else
		std::cout << "minimum path cover confirmed" << std::endl;

	computeMPCIndex(opt);
	std::cout << "MPCIndex is built" << std::endl;

	if (output_filename.empty()) output_filename = "out.txt";
	std::ofstream fout(output_filename);
	for (const AnchorSet &anchors : anchor_sets) {
		// std::cout << "now anchor set: " << anchors.name << ", size : " << anchors.anchors.size() << std::endl;
		std::vector<LL> chain_ids = sparseDP(opt, anchors.anchors);
		Chain chain;
		for (LL i : chain_ids)
